...
import {TiWeatherCloudy, TiWeatherDownpour, TiWeatherSnow, TiWeatherStormy, TiWeatherSunny} from "react-icons/ti";

const InternalHome = observer(() => {
    const weatherData = api.weather.getWeather.useQuery({coordinates: {lat: 52.116, lon: 13.622}}, { refetchOnWindowFocus: false});
    let temperature = undefined;
    if (weatherData.data?.temperature) {
        temperature = temperatureUnit$.get() === "Celsius" ? `${Math.round(weatherData.data?.temperature - 273.15)}°C` : `${Math.round(weatherData.data?.temperature * 9 / 5 - 459.67)}°F`;
    }

    type WeatherStateType = "Sunny" | "Cloudy" | "Rainy" | "Stormy" | "Snowy" | JSX.Element | undefined;
  ...
  return (
    <Layout>
        <div className="mt-24 flex items-center flex-col">
            ...
            <h1 className="text-7xl mt-3 text-gray-500">{temperature ? temperature : "Loading..."}</h1>
            <p className="mt-3 text-xl">{weatherState({hour: 0, icons: false}) ? weatherState({hour: 0, icons: false}): "Loading..."}</p>
        </div>
            ...
            {weatherData.data?.hourlyForecast.map((hourlyForecast, index) => {
                ...
                return (
                    <div className="m-3 flex flex-col items-center" key={index}>
                        ...
                        <div>{temperatureUnit$.get() === "Celsius" ? `${Math.round(hourlyForecast.temperature - 273.15)}°C` : `${Math.round(hourlyForecast.temperature * 9 / 5 - 459.67)}°F`}</div>
                        ): "Loading"}
                    </div>
                )
            })}
